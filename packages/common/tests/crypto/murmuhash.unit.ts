...
describe('hash()', () => {
  it('return a 32-bit hash resulting from the murmurhash3 algorithm', () => {
    expect(MurmurHash.hash('hello', 0)).to.eql('FIEEdaEG');
    expect(MurmurHash.hash('hello world', 0)).to.eql('EEeBEJAF');
    expect(MurmurHash.hash('helloworld', 0)).to.eql('BIFECFfG');
    expect(MurmurHash.hash('HELLOWORLD', 0)).to.eql('dIBEIJda');
    expect(MurmurHash.hash('', 0)).to.eql('A'); // not sure why '0000 0000' fails as a test
    expect(MurmurHash.hash('abc', 0)).to.eql('bDddJDfa');
    expect(MurmurHash.hash('ABC', 0)).to.eql('EDcBcFDF');
