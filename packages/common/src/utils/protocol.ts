...
export const createAsyncAction = <P, R, E extends AsyncError<number> = AsyncError>(
  type: string,
  commonMeta?: Meta
): AsyncActionCreators<P, R, E> => createAction.async(type, commonMeta);
...
export class Channel<K extends string> {
  constructor(
    public variables: K[],
    public build: (params: Record<K, string>) => string
  ) {}

  buildMatcher() {
    return this.build(
      this.variables.reduce((acc, key) => Object.assign(acc, { [key]: `:${key}` }), {} as Record<K, string>)
    );
  }
...
export const createChannel = <K extends string>(variables: K[], build: (params: Record<K, string>) => string) =>
  new Channel(variables, build);
